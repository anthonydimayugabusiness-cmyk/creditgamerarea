...
<body>
    ...
    <div id="error-msg" class="error-message" style="max-width: 700px; margin: 40px auto;">
        ...
        <a href="/credit-basics-quiz.html" class="next-btn">Go to Quiz Start</a>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (QUESTION_NUM === 1) {
                sessionStorage.setItem('cb_score', '0');
                return;
            }
            ...
            if (!params.token || !QuizToken.validate(params.token, QUIZ_NAME, QUESTION_NUM - 1, params.score)) {
                ...
                sessionStorage.setItem('cb_score', params.score.toString());
            }
        });
        
        let answered = false;
        let currentScore = parseInt(sessionStorage.getItem('cb_score') || '0');
        ...
        function selectAnswer(selected, correct) {
            ...
            if (isCorrect) {
                currentScore++;
                sessionStorage.setItem('cb_score', currentScore.toString());
            }
        }
        ...
        function goToNext() {
            if (IS_LAST) {
                ...
                const token = QuizToken.generate(QUIZ_NAME, 'complete', currentScore);
                window.location.href = QuizToken.buildUrl('/quiz-complete.html', QUIZ_NAME, 'complete', currentScore);
            } else {
                // Go to next question with token
                const nextUrl = QuizToken.buildUrl('${next_page}', QUIZ_NAME, QUESTION_NUM + 1, currentScore);
...
EOF
    ...
    echo "Created q${q_num}.html with token protection"
done

echo "All ${total} token-protected question pages created!"
