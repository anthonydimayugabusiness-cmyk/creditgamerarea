#!/usr/bin/env python3
"""Update all quiz question pages with the proper 4-column footer."""
...
def update_quiz_page(filepath):
    """Update a quiz page with the proper footer."""
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Check if it has the simple footer
    if '<footer class="footer" style="background: #0f0d2e; color: white; padding: 40px 0; text-align: center;">' in content:
        # Replace simple footer with 4-column footer
        content = re.sub(
            r'<footer class="footer" style="background: #0f0d2e; color: white; padding: 40px 0; text-align: center;">\s*<div class="container">\s*<p>&copy; 2026 Credit Gamer Area\. All rights reserved\. \| <a href="/privacy\.html" style="color: #a5b4fc;">Privacy</a> \| <a href="/terms\.html" style="color: #a5b4fc;">Terms</a></p>\s*</div>\s*</footer>',
            FOOTER_HTML,
            content
        )
...
def main():
    ...
    for filepath in quiz_pages:
        if update_quiz_page(filepath):
            updated += 1
            print(f"Updated: {filepath}")
    
    print(f"\nUpdated {updated} quiz pages with 4-column footer")
